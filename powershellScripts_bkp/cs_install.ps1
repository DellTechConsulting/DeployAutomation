...
$filepath = "E:\Workspace\UIAutomation\Statusfiles\CSDeploymentStatus.txt"
try{
...
if ($LoginCredentials) {
  ...
  if ($MyServer) {
    ...
    if ($checkOva) {
       ...
       Start-VM -VM $VMName
       sleep -s 90
       Write-Log "E:\Workspace\UIAutomation\Log Files\CRLog.txt" "[INFO] CS Deployment is successful" 
       get-vm -Name $VMName | % {
                $vm = Get-View $_.ID
