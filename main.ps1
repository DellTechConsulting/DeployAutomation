...
function Get-CR-Install-Json($FormInput) {
    ...
    if ($deploymentType -like 'vCenter') {
        ...
        $jsonBase.Add("logfile", ($FormInput[21] -split "=")[1].replace("%20", " ")) 
    }
    else {
        ...
        $jsonBase.Add("crdata", $crdata)
        $jsonBase.Add("logfile", ($FormInput[19] -split "=")[1].replace("%20", " ")) 
    }
...
function Get-CS-Install-Json ($FormInput) {
    ...
    if ($deploymentType -like 'vCenter') {
        ...
        $jsonBase.Add("logfile", ($FormInput[25] -split "=")[1].replace("%20", " ")) 
    }
    else {
        $esxiHost = ($FormInput[1] -split "=")[1].replace("%20", " ")
        $username = ($FormInput[2] -split "=")[1].replace("%20", " ")
        ...
        $jsonBase.Add("csdata", $csdata)
        $jsonBase.Add("logfile", ($FormInput[23] -split "=")[1].replace("%20", " ")) 
    }
...
function Get-AVE-Install-Json ($FormInput) {
    ...
    if ($deploymentType -like 'vCenter') {
        ...
        $jsonBase.Add("logfile", ($FormInput[28] -split "=")[1].replace("%20", " ")) 
    }
    else {
        $esxiHost = ($FormInput[1] -split "=")[1].replace("%20", " ")
        $username = ($FormInput[2] -split "=")[1].replace("%20", " ")
        ...
        $jsonBase.Add("avedata", $avedata)
        $jsonBase.Add("logfile", ($FormInput[26] -split "=")[1].replace("%20", " ")) 
...
function Get-DDVE-Install-Json ($FormInput) {
    ...
    $jsonBase.Add("logfile", ($FormInput[13] -split "=")[1].replace("%20", " ")) 
...
function Get-Networker-Install-Json ($FormInput) {
    ...
    if ($deploymentType -like 'vCenter') {
        ...
        $jsonBase.Add("logfile", ($FormInput[18] -split "=")[1].replace("%20", " ")) 
        ...
        $jsonBase.Add("networkerdata", $networkerdata)
        $jsonBase.Add("logfile", ($FormInput[16] -split "=")[1].replace("%20", " ")) 
    }
...
function Get-Proxy-Install-Json ($FormInput) {
    ...
    if ($deploymentType -like 'vCenter') {
        ...
        #$fqdn = ($FormInput[8] -split "=")[1].replace("%20", " ")
        $dns = ($FormInput[8] -split "=")[1].replace("%20", " ")
        $DiskStorageFormat = ($FormInput[9] -split "=")[1].replace("%20", " ")
        $Network = ($FormInput[10] -split "=")[1].replace("%20", " ")
        ...
        $ovaPath = ($FormInput[5] -split "=")[1].replace("%20", " ")
        #$fqdn = ($FormInput[6] -split "=")[1].replace("%20", " ")
        $dns = ($FormInput[6] -split "=")[1].replace("%20", " ")
        $DiskStorageFormat = ($FormInput[7] -split "=")[1].replace("%20", " ")
        $Network = ($FormInput[8] -split "=")[1].replace("%20", " ")
        ...
        $jsonBase.Add("proxydata", $proxydata)
        $jsonBase.Add("logfile", ($FormInput[14] -split "=")[1].replace("%20", " ")) 
...
function Get-Networker-Proxy-Install-Json  ($FormInput) {
    $jsonBase = @{}
    ...
    if ($deploymentType -like 'vCenter') {
        ...
        $ntp = ($FormInput[16] -split "=")[1].replace("%20", " ")
        $timezone = ($FormInput[17] -split "=")[1].replace("%20", " ")
        $defaultRootpassword = ($FormInput[18] -split "=")[1].replace("%20", " ")
        $defaultAdminpassword = ($FormInput[19] -split "=")[1].replace("%20", " ")
        

        $networkerproxydata = @([pscustomobject]@{"VMName" = $VMName
                "ovaPath"                        = $ovaPath
                "hostData"                       = $hostData
                "fqdn"                           = $fqdn
                "DiskStorageFormat"              = $DiskStorageFormat
                ...
                "ntp"                            = $ntp
                "timezone"                       = $timezone
                "rootpass"                       = $defaultRootpassword
                "adminpass"                      = $defaultAdminpassword
            })

        $jsonBase.Add("nveproxydata", $networkerproxydata)
        
        $jsonBase.Add("logfile", ($FormInput[20] -split "=")[1].replace("%20", " ")) 
    }
    else {
        $esxiHost = ($FormInput[1] -split "=")[1].replace("%20", " ")
        $username = ($FormInput[2] -split "=")[1].replace("%20", " ")
        ...
        $ntp = ($FormInput[14] -split "=")[1].replace("%20", " ")
        $timezone = ($FormInput[15] -split "=")[1].replace("%20", " ")
        $defaultRootpassword = ($FormInput[16] -split "=")[1].replace("%20", " ")
        $defaultAdminpassword = ($FormInput[17] -split "=")[1].replace("%20", " ")
...
function Get-NTP-Install-Json ($FormInput) {
    ...
    $jsonBase.Add("logfile", ($FormInput[7] -split "=")[1].replace("%20", " ")) 
...
function Get-DNS-Install-Json ($FormInput) {
    ...
    $jsonBase.Add("logfile", ($FormInput[6] -split "=")[1].replace("%20", " ")) 
...
switch ($TestInput1) {
    "cr-install" {
        ...
        .\powershellScripts\cr_install.ps1 -FormInput $crInputJson
        Break
    }
    "cs-install" {
        $csInstallInputJson = Get-CS-Install-Json $FormInputArray
        .\powershellScripts\cs_install.ps1 -FormInput $csInstallInputJson
        Break
    }
    "cs-integrate" {
    ...
    "ave-install" {
        ...
        .\powershellScripts\ave_install.ps1 -FormInput $aveInstallInputJson
        Break
    }
    "ave-integrate" {
    ...
    "ddve-install" {
        ...
        .\powershellScripts\ddve_install.ps1 -FormInput $ddveInstallInputJson
        Break
    }
    "ddve-integrate" {
    ...
    "networker-install" {
        ...
        .\powershellScripts\networker_install.ps1 -FormInput $networkerInstallInputJson
        Break
    }
    "proxy-install" {
        $proxyInstallInputJson = Get-Proxy-Install-Json $FormInputArray
        .\powershellScripts\proxy_install.ps1 -FormInput $proxyInstallInputJson
        Break
    }
    "ppdm-install" {
        $ppdmInstallInputJson = Get-PPDM-Install-Json $FormInputArray
        .\powershellScripts\ppdm_install.ps1 -FormInput $ppdmInstallInputJson
        Break
    }
    "networker-proxy-install" {
        $networkerProxyInstallInputJson = Get-Networker-Proxy-Install-Json $FormInputArray
        .\powershellScripts\networker_proxy_install.ps1 -FormInput $networkerProxyInstallInputJson
        Break
    }
    "ntp-install" {
        $ntpInstallInputJson = Get-NTP-Install-Json $FormInputArray
        .\powershellScripts\ntp_install.ps1 -FormInput $ntpInstallInputJson
        Break
    }
    "dns-install" {
